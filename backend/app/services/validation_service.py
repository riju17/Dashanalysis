...
def validate_team_payload(payload: dict, team_id: Optional[str] = None):
...
def validate_venue_payload(payload: dict, venue_id: Optional[str] = None):
...
def validate_player_payload(payload: dict):
    ...
    if not store.get("teams", str(payload["team_id"])):
        _raise("team_id does not exist")


def validate_match_payload(payload: dict):
    ...
    if not store.get("teams", str(payload["team_a_id"])) or not store.get("teams", str(payload["team_b_id"])):
        _raise("Team ids must exist")
    if not store.get("venues", str(payload["venue_id"])):
